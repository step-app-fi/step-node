...
if [ -z ${ROCKSDBALLOWED+x} ]; then
    ...
    go build -ldflags "$static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
else
    echo "Building AvalancheGo with rocksdb enabled..."
    go build -tags rocksdballowed -ldflags "$static_ld_flags" -o "$avalanchego_path" "$AVALANCHE_PATH/main/"*.go
fi
